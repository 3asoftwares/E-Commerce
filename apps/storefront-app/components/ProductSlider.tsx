...
export function ProductSlider({ 
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (width < 640) {
        setResponsive(1); // mobile
      } else if (width < 768) {
        setResponsive(2); // tablet
      } else if (width < 1024) {
        setResponsive(3); // small desktop
      } else {
        setResponsive(itemsPerView); // large desktop
      }
    };
      ...
      <div className="overflow-hidden">
        ...
        >
          {Array.from({ length: Math.ceil(totalItems / responsive) }).map((_, slideIndex) => (
            ...
            >
              {children.slice(
                slideIndex * responsive,
                slideIndex * responsive + responsive
              )}
            </div>
          ))}
        </div>
