...
describe('MultiSelect', () => {
  ...
  it('removes value when badge close button is clicked', () => {
    ...
    const closeBadgeButtons = screen.getAllByText('×');
  ...
  it('applies correct variant classes', () => {
    ...
    expect(container.firstChild?.firstChild).toHaveClass('border', 'border-gray-300', 'bg-white');
  ...
  it('disables specific options', async () => {
    ...
    const checkboxes = screen.getAllByRole('checkbox');
    // The last option (index 3) should be disabled
  ...
  it('toggles selected values correctly', async () => {
    ...
    // Check that the badge is shown for selected option
    expect(screen.getByText('Option 1')).toBeInTheDocument();
    
    // Remove the selected option by clicking the × button
